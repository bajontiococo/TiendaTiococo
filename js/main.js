...
window.addEventListener('DOMContentLoaded', () => { 
    ...
    // 1. Cargar Edge-to-Edge si estamos en el index
...
async function cargarDestacadosEdgeToEdge() {
    ...
    if (!container) return; // Si no estamos en el index.html, no hace nada

    try {
        // Traemos solo los productos con destacado = 1, máximo 3
        ...
        // Si no hay productos destacados, mostramos un mensaje por defecto
        if (!data || data.length === 0) {
            container.innerHTML = `<div class="w-full flex items-center justify-center text-white"><p>No hay destacados configurados.</p></div>`;
            return;
        }

        // Generamos los 3 bloques. Flex-1 hace que se repartan el espacio equitativamente.
...
function buscar(query) {
    ...
    if (!contenedorGrid) {
        ...
        }, 800); // Espera 800ms para dejar que el usuario escriba
        return;
    }
...
async function procesarCheckout() {
    ...
    if(!currentUser) { 
        mostrarToast("Debes iniciar sesión para pedir", "info"); 
        openAuthModal(); 
        return; 
    }
    if (!userProfile) { 
...
async function cambiarEstadoPedido(id, nuevoEstado) {
    ...
    if (error) { 
        ...
        mostrarToast(`Pedido #${id} actualizado`, "success"); 
        cargarPedidosLogistica(); 
    }
}
...
function initSwipeGestures() {
    ...
    document.addEventListener('touchend', function(e) {
        ...
        if (diffY < 30 && Math.abs(diffX) > 50) {
            ...
            if (diffX > 0 && sidebar && sidebar.classList.contains('translate-x-0')) { 
                forceCloseCart(); 
            } else if (diffX < 0 && leftSidebar && !leftSidebar.classList.contains('-translate-x-full')) { 
                handleBackAction(); 
            }
        }
    }, {passive: true});
